        ...
        <Box id={'Section' + props.sectionNum} className='mb-2 mt-4 pb-3' bgcolor='primary.main' borderRadius={1}>
            ...
            <Box className="me-0 d-flex justify-content-between align-items-center pe-3" sx={{ borderRadius: '8px', display: 'flex',height:'60px'}}>
                <div className='d-flex w-75'> 
                    <TextField name={'Section' + props.sectionNum + 'sectionName'} variant='standard' className=" bg-transparent text-white ms-3 my-auto" placeholder={sectionTitle} onChange={(event) => setSectionTitle(event.target.value)} fullWidth required></TextField>
                </div>
            ...
            <Box display='flex'>
                ...
                <FormControl sx={{textAlign:"start"}}>
                    <Select name={'Section' + props.sectionNum + 'assignedTo'} sx={{ height: '50px', width: 200, ms: '3' }} value={assignedTo} onChange={(event) => handleChangeAssignedTo(event.target.value)}>
