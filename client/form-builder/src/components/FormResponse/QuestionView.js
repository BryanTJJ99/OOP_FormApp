...
const QuestionView = (props) => {
    const [dropdownVal, setDropdownVal] = useState(
        props.response.formAnswer[props.question.questionOrder]
    );
    ...
    const [ratingValue, setRatingValue] = useState(
        parseInt(props.response.formAnswer[props.question.questionOrder])
    );
    ...
    useEffect(() => {
        ...
        if (
            props.question.questionOrder in props.response.formAnswer &&
            props.question.questionType === "file"
        ) {
            setFile(props.response.formAnswer[props.question.questionOrder][0]);
            props.handleFileUpload(props.question.questionOrder, [
                "current",
                props.response.formAnswer[props.question.questionOrder][0],
                props.response.formAnswer[props.question.questionOrder][1],
            ]);
        }
    }, []);
    ...
    const handleRadioChange = (event) => {
        if (!props.disabled) {
            ...
            response.formAnswer[props.question.questionOrder] =
                event.target.value;
    ...
    const handleCheckboxChange = (event) => {
        ...
        if (newCheckboxValue.includes(event.target.value)) {
            newCheckboxValue.splice(
                newCheckboxValue.indexOf(event.target.value),
                1
            );
        } else {
        ...
        response.formAnswer[props.question.questionOrder] = newCheckboxValue;
        props.setFormResponse(response);
    };

    const handleRatingChange = (event) => {
        setRatingValue(event.target.value);
        response.formAnswer[props.question.questionOrder] = event.target.value;
        props.setFormResponse(response);
    };
    ...
    function specialQuestionType(questionType) {
        if (questionType === "text") {
            ...
            for (let i = 0; i < props.question.choices.length; i++) {
                choices.push(
                    <FormControlLabel
                        ...
                        label={props.question.choices[i]}
            ...
            for (let i = 0; i < props.question.choices.length; i++) {
                let checked = false;
                if (
                    Object.keys(props.response.formAnswer).includes(
                        props.question.questionOrder.toString()
                    )
                ) {
                    if (
                        Object.values(
                            props.response.formAnswer[
                                props.question.questionOrder
                            ]
                        ).includes(i.toString())
                    ) {
                        checked = true;
            ...
            for (let i = 0; i < props.question.choices.length; i++) {
                choices.push(
                    <MenuItem value={i}>{props.question.choices[i]}</MenuItem>
                );
            }
            return (
                <div className="w-50">
            ...
            // console.log(file)
            console.log(
                Object.keys(props.response.formAnswer),
                props.question.questionOrder
            );
            if (
                Object.keys(props.response.formAnswer).includes(
            ...
            return <div className="d-flex">{fileElement}</div>;
        }
    }
